...
namespace HolePlugin
{
    ...
    public class AddHole : IExternalCommand
    {
        ...
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            {
                TaskDialog.Show("Ошибка", "Не найдено семейство \"Овтерстия\"");
                return Result.Cancelled;
            }
            ...
            if (view3D == null)
            {
                TaskDialog.Show("Ошибка", "Не создан 3D вид");
                return Result.Cancelled;
            }
            ...
            Transaction transaction1 = new Transaction(arDoc, "Активация семейства");
            transaction1.Start();
            ...
            Transaction transaction = new Transaction(arDoc, "Вставка отверстия");
            transaction.Start();

            foreach (Pipe p in pipes)
            {
                Line curve1 = (p.Location as LocationCurve).Curve as Line;
                XYZ point1 = curve1.GetEndPoint(0);
                XYZ direction1 = curve1.Direction;

                List<ReferenceWithContext> intersections = referenceIntersector.Find(point1, direction1)
                    .Where(x => x.Proximity < curve1.Length)
                    .Distinct(new ReferenceWithContextElementEqualityComparer())
                    .ToList();

                foreach (ReferenceWithContext refer1 in intersections)
                {
                    double proximity1 = refer1.Proximity;
                    Reference reference = refer1.GetReference();

                    Wall wall1 = arDoc.GetElement(reference.ElementId) as Wall;

                    Level level = arDoc.GetElement(wall1.LevelId) as Level;
                    XYZ pointHole1 = point1 + (direction1 * proximity1);
